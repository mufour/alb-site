    <header>
        <div class="menu">
            <div class="logo">
                <a href="https://www.amicalebacalan.com">
                    <img src="images/logo-bacalan.png" alt="logo-bacalan" width="55" height="55">
                </a>
            </div>
            <div class="menu-container">
                <button class="burger-btn" id="burger-btn" aria-label="Ouvrir le menu"><img src="images/burger-logo.png"
                        alt="burger-logo"></button>
                <nav class="main-nav">
                    <ul class="nav-links" id="nav-links">
                        <li class="text-accueil"><a href="https://www.amicalebacalan.com/">Accueil</a></li>
                        <li class="text-amicale"><a href="/presentation">L'Amicale</a>
                        </li>
                        <li class="text-activities"><a href="/ateliers">Activités</a>
                        </li>
                        <li class="text-partners"><a href="/associatifs">Partenaires</a>
                        </li>
                        <li class="text-contact"><a href="/contacts">Contact</a></li>
                    </ul>
                </nav>
            </div>
        </div>
    </header>